	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/04_nuthatch_recomb/04_stats"
	directory_name <- "wbnu_stats_100kbp"
	cluster <- "quanah"
	max_number_jobs <- 400
	
	# read in reference index
	# filtered to only include genotyped chromosomes
	ref_index <- read.table("wbnu.fasta.fai", stringsAsFactors=F)
	...
	write(paste("#SBATCH --job-name=", "stat_wbnu", sep=""), file=a.script, append=T)
	...
	header <- paste('\tgunzip -cd ', project_directory, '/Scaffold_39.recode.vcf.gz | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
